module Jekyll
  ...
  class CatsAndTags < Generator
    ...
    def paginate(site, type, posts)
      ...
      (1..pages).each do |num_page|
        path = "/#{type}/#{posts[0]}"
        pager = Pager.new(site.config, num_page, posts[1], path + '/', '', pages)

        if num_page > 1
          path = path + "/page/#{num_page}"
        end

        newpage = GroupSubPage.new(site, site.source, path, type, posts[0], "group_index.html")
        newpage.pager = pager
        site.pages << newpage 
  ...
  class GroupSubPage < Page
    def initialize(site, base, dir, type, val, template)
      @site = site
      @base = base
      @dir = dir
      @name = 'index.html'
