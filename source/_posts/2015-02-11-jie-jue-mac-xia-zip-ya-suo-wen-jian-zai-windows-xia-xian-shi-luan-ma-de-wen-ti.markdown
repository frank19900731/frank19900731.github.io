...
1. [Keka](http://www.kekaosx.com/en/)，可行。
2. 上传到百度文库然后下载，无效。
...
第 5 种方案 7z 命令行是可行的，下一段中会介绍。其实 **Keka 也是基于 7z 压缩的**，一方面可见 `/Applications/Keka.app/Contents/Resources` 目录下的可执行文件，另一方面，我对比了 Keka 和 7z 对同一个文本文件的压缩结果，**二进制表示完全一致**。
